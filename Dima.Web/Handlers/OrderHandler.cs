...
public class OrderHandler(IHttpClientFactory httpClientFactory) : IOrderHandler
{
    ...
    {
        var response = await _client.PostAsJsonAsync($"v1/orders/{request.Id}/cancel", request);
    ...
    {
        var response = await _client.PostAsJsonAsync($"v1/orders/{request.Id}/pay", request);
    ...
    {
        var response = await _client.PostAsJsonAsync($"v1/orders/{request.Id}/refund", request);
